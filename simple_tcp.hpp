...
class simple_tcp {
 ...
 public:
  simple_tcp(std::string address, int port)
      : sockfd(-1), client_sockfd(-1), is_server_(false) {
  ...
  int create_client() {
    ...
    int keepalive = 1;
    setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, &keepalive,
               sizeof(keepalive));
  ...
  bool SendFramed(const uint8_t *src, uint32_t len) {
    ...
    if (!send_all(reinterpret_cast<const uint8_t *>(&len_be), 4)) return false;
    return send_all(src, len);
  }

  bool RecvFramed(std::vector<uint8_t> &dst, uint32_t &len) {
    uint32_t len_be = 0;
    if (!recv_all(reinterpret_cast<uint8_t *>(&len_be), 4)) return false;
    len = ntohl(len_be);
  ...
  void Tx(uint8_t *src, size_t len) { send_all(src, len); }
};
